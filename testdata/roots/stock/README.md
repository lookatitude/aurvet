...
| `…/foo.service`'s `ExecStartPre=-fooplymouth --wait quit` | The reference system's **dominant** shape: 6 of the 8 `unit-coverage` gaps measured there were `ExecStartPre=-plymouth --wait quit` in systemd's *own* units. The leading `-` is systemd syntax for "this command is optional and its failure is ignored", and the command is absent from this root. Neither a gap nor a finding — the check ran and got a determinate answer, and reporting it as "could not be examined" states something false (INV-9 cuts both ways). The `-` suppresses **only** this conjunction: a `-`-prefixed command that *does* resolve gets the ordinary ownership verdict, or an attacker buys an exemption for one character. |
